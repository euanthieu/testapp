...
const HeaderCategoryItem = React.memo(({title, onPress, category}) => {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        {
          borderBottomColor: title === category ? colors.primary : colors.white,
        },
      ]}>
      <ZText
        type="b18"
...
export default function ReelsComponent() {
  ...
  const BASE_URL = Platform.OS === 'android' ? API_BASE_URL.replace('localhost', '10.0.2.2') : API_BASE_URL;

  const fetchFeed = async (nextPage = 0) => {
    if (loading || !hasMore) return;
    setLoading(true);
    try {
      const response = await fetch(`${BASE_URL}/api/v1/feed?page=${nextPage}&limit=5`);
      const result = await response.json();
      console.log('API Result:', JSON.stringify(result, null, 2));
      if (result.success) {
        const transformedData = result.data.map(item => ({
          id: item.id,
          uri: item.content.file.fileUrl,
          poster: item.content.file.fileUrl, // Using fileUrl as poster, can be replaced
          user_name: item.content.artist.name,
          description: item.content.meta_data.caption,
          music: 'Original Sound',
          like: '0',
          comment: '0',
          isLike: false,
        }));
        setFeed(prevFeed => (nextPage === 0 ? transformedData : [...prevFeed, ...transformedData]));
  ...
  }, [isFocused, musicToneAnimated, musicToneAnimated1]);

  useEffect(() => {
    setExtraData(!extraData);
  }, [isMute, isLike, activeVideoIndex, isFocused, category]);
  ...
  const onPressShare = () => {
    Share.share({
      message: 'Tikto App | Entertainment, Social, Music, Video, Live, Chat...',
    });
  };
          ...
          <View style={localStyles.bottomContainer}>
            <View style={styles.flex}>
              <View style={styles.rowCenter}>
                <TouchableOpacity
                  onPress={() =>
                    onPressProfile(item?.user_name, item?.avatarUri)
                  }
                  style={styles.mb10}>
                  <Image
                    resizeMode="cover"
