    ...
    try {
      ...
      if (!!asyncData) {
        ...
        if (!!access_token) {
          navigation.reset({
            index: 0,
            routes: [{name: StackNav.Auth, params: {screen: StackNav.LoginSuccess}}],
          });
        } else {
          // Always go to Login inside Auth, ignore OnBoarding
          // If appOpenFirstTime, go to Auth, else go to OnBoarding
          //   if (!!appOpenFirstTime) {
          //   navigation.reset({
          //     index: 0,
